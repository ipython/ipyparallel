...
senders = [
    ("windows", shellcmd.ShellCommandSend(["cmd.exe"], ["/C"], sys.executable, initialize=False), None),
    ("windows", shellcmd.ShellCommandSend(["cmd.exe"], ["/C"], sys.executable, initialize=False, send_receiver_class=1),
     None),
    ("windows", shellcmd.ShellCommandSend(["powershell.exe"], ["-Command"], sys.executable, initialize=False), None),
    ("windows", shellcmd.ShellCommandSend(["powershell.exe"], ["-Command"], sys.executable, initialize=False,
                                          send_receiver_class=1), None),
    ("windows",
     shellcmd.ShellCommandSend(["ssh"], ["-p", "2222", "ciuser@127.0.0.1"], windows_py_path, initialize=False), None),
    ("windows",
    ...
    ("wsl", shellcmd.ShellCommandSend(["bash"], ["-c"], "python3", initialize=False, send_receiver_class=1),
     "/home/jo/"),
    ("linux", shellcmd.ShellCommandSend(["/usr/bin/bash"], ["-c"], "python3", initialize=False), None),
...
@pytest.mark.parametrize("platform, sender, prefix", senders, ids=sender_ids)
def test_shellcmds(platform, sender, prefix, shellcmd_test_cmd):
